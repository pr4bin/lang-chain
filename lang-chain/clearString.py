...
def _(text: List[str]) -> str: # Correct return type hint
    """Removes citations and bold markdown from a list of strings and returns a formatted string."""
    # --- OPTIMIZATION/STYLE: Use list comprehension ---
    cleaned_list_elements = [
        remove_citations_and_bold(item) if isinstance(item, str) else str(item)
        for item in text
    ]
    return f"[{', '.join(cleaned_list_elements)}]"